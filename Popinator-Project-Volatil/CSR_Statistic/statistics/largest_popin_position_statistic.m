function largest_popin_position_statistic(data,path,filtertext)
    ...
    % export data (header)
    fileout_data = "Statistic"+filtertext+"_LargestPopinPosition_All.txt";
    ...
    for n = 1:datasets
        ...
        stats{temp,mat,rate} = [stats{temp,mat,rate}, [data{n,2}(idx,1); data{n,2}(idx,2)]]; 
    ...
    fileout_avg = "Statistic"+filtertext+"_LargestPopinPosition_Average.txt";
    fid = fopen(path + fileout_avg,'w');
    fprintf(fid,'%s %s %s %s %s %s %s\n','Average_Displacement_[nm]','Average_Load_[µN]','Standart_Error_Displacement','Standart_Error_Load','Strainrate','Temperature','Material');
    ...
    fileout_graph = "Statistic"+filtertext+"_LargestPopinPosition.png";
