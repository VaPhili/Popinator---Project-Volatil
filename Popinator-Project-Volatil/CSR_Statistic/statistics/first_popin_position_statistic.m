function first_popin_position_statistic(data,path,filtertext)
    ...
    % export data (header)
    fileout_data = "Statistic"+filtertext+"_FirstPopinPosition_All.txt";
    fid = fopen(path + fileout_data,'w');
    fprintf(fid,'%s %s %s %s %s %s\n','Displacement [nm]','Load [µN]','Material','Temperature','Strainrate','Number');
    ...
    for n = 1:datasets
        ...
        stats{temp,mat,rate} = [stats{temp,mat,rate}, [data{n,2}(1,1); data{n,2}(1,2)]]; 

        % export to file
        fprintf(fid,'%5.5f \t %5.5f \t %8s         %3d       %2.2f  %5s \n',data{n,2}(1,1), data{n,2}(1,2), data{n,1}{1,1}, data{n,1}{1,2}, rate, data{n,1}{1,4}); 
    end
    ...
    fileout_avg = "Statistic"+filtertext+"_FirstPopinPosition_Average.txt";
    fid = fopen(path + fileout_avg,'w');
    fprintf(fid,'%s %s %s %s %s %s %s\n','Average_Displacement [nm]','Average_Load [µN]','Standart_Error Displacement','Standart_Error Load','Strainrate','Temperature','Material');

    % Sets averages
    avg_stats = zeros(size(temperatures,1),size(materials,1),size(rates,1),4);
    for temp = 1:size(temperatures,1)
        for mat = 1:size(materials,1)
            for rate = 1:size(rates,1)

                if size(stats{temp,mat,rate},2) == 0
                    continue
                end

                % mean
                avg_stats(temp,mat,rate,1) = mean(stats{temp,mat,rate}(1,:));
                avg_stats(temp,mat,rate,2) = mean(stats{temp,mat,rate}(2,:));
                % standart error
                avg_stats(temp,mat,rate,3) = std(stats{temp,mat,rate}(1,:)) / sqrt(size(stats{temp,mat,rate},2));
                avg_stats(temp,mat,rate,4) = std(stats{temp,mat,rate}(2,:)) / sqrt(size(stats{temp,mat,rate},2));

                fprintf(fid,'%5.5f\t\t%5.5f\t\t%3.4f\t\t%3.4f\t\t%2.2f\t  %3d\t   %8s\n',avg_stats(temp,mat,rate,1), avg_stats(temp,mat,rate,2), avg_stats(temp,mat,rate,3), avg_stats(temp,mat,rate,4), rates(rate), temperatures(temp), materials(mat));
            end
        end
    end
    fclose(fid);
    ...
    for temp = 1:size(temperatures,1)
        for mat = 1:size(materials,1)
            ...
            for rate = 1:size(stats,3)
                if size(stats{temp,mat,rate},2) == 0
                    ...
                    tmp_disp = stats{temp, mat, rate}(1,:);
                end
                tmp_rate = rates(rate) * ones(size(tmp_disp(:)));
                tmp = [tmp; [tmp_rate, tmp_disp(:)]];
            end
        ...
        for mat = 1:size(materials,1)
            ...
            err = errorbar(rates(:),squeeze(avg_stats(temp,mat,:,1)),squeeze(avg_stats(temp,mat,:,3)));
    ...
    fileout_graph = "Statistic"+filtertext+"_FirstPopinPosition.png";
